...
// Gus549NonStupid implements the "Pippenger approach" from Section 4 of
// https://eprint.iacr.org/2012/549.pdf
// Non-stupid: avoid quadratic dependence on t by iterating from t-1 down to 0
func (p *G1Jac) Gus549NonStupid(curve *Curve, points []G1Jac, scalars []fr.Element, c int) *G1Jac {
	// const c int = 4                        // scalars partitioned into c-bit radixes, must divide 64
	t := fr.ElementLimbs * 64 / c        // number of c-bit radixes in a scalar
	selectorMask := uint64((1 << c) - 1) // low c bits are 1
